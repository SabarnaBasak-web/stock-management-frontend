...
interface IRegisterProps {
  updateMonitorDetails?: IMonitorResponse | null;
  cancelUpdate: () => void;
}

interface IRegisterMonitorInitialValues {
...
function RegisterMonitor(props: IRegisterProps) {
  const { updateMonitorDetails, cancelUpdate } = props;
  ...
  const registerMonitorInitialValue: IRegisterMonitorInitialValues = {
    gemNo: "",
    brandName: "",
    serialNo: "",
    modelNo: "",
  ...
  const formik = useFormik<IRegisterMonitorInitialValues>({
    // initialValues: updateUpsDetails
    //   ? selectedUpsDetails(updateUpsDetails)
    //   :
    initialValues: registerMonitorInitialValue,
    validationSchema: Yup.object({
      ...
      displaySize: Yup.string().required("Required"),
    }),
    onSubmit: (values) => {
      const requestPayload: IMonitorRequestPayload = {
        gemNo: +values.gemNo,
      ...
      //   );
      // } else {
      dispatch(registerMonitor(requestPayload));
      // }
      resetForm({ values: { ...registerMonitorInitialValue } });
    },
  ...
  return (
    <>
      <form onSubmit={handleSubmit}>
        <Card sx={{ width: "350px" }}>
          <CardHeader
            title={updateMonitorDetails ? "Update Monitor" : "Register Monitor"}
          />
          <StyledCardContent>
            <TextField
              id='gemNo'
            ...
            />
            {errors.displaySize && touched.displaySize ? (
              <ErrorText text={errors.displaySize ?? ""} />
            ) : null}
            {/* Vendor */}
            {vendorsList.length && (
          ...
          >
            <Button variant='contained' type='submit'>
              {updateMonitorDetails ? "Update" : "Register"}
            </Button>
            {updateMonitorDetails ? (
